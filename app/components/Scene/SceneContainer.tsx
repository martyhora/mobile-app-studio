...
export default class SceneContainer extends React.Component<ISceneContainerProps, ISceneContainer> {
    ...
    addSection(): void {
        ...
        if (sectionTitle === '' || newSection === '' || sections.filter(section => section.title === sectionTitle).length > 0) {
            return;
        }

        let sections: Array<ISection> = sections;

        let section: ISection = { title: sectionTitle, type: newSection, marginTop: 0, marginBottom: 0, link: '' };
    ...
    componentDidMount() {
        ...
        SceneApi.fetchScenesForApplicationBySceneId(this.props.match.params.id, (scenes: Array<IScene>) => {
